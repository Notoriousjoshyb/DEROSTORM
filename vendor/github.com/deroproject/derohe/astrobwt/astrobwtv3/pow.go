...
// RecoveredPanics counts hashes aborted by the recover above. It must stay at
// zero: a non-zero value means a hash silently returned a falsified result.
// The miner surfaces it, and the test suite asserts it after bulk runs. This
// also validates building this package with bounds checks disabled (-B), which
// is only sound while this counter provably stays zero.
...
// AstroBWTv3 keeps the original API: it borrows a scratch buffer from the pool.
func AstroBWTv3(input []byte) (outputhash [32]byte) {
	scratch := Pool.Get().(*ScratchData)
	outputhash = AstroBWTv3_scratch(input, scratch)
	Pool.Put(scratch)
	return
}

// AstroBWTv3_scratch is the hot-path entry point: the caller owns the scratch
...
// AstroBWTv3_pair hashes two inputs and shares one paired SHA-256 between them.
//
// Each digest is exactly what AstroBWTv3_scratch would return for that input.
// The only thing shared is the schedule of the final SHA-256, which is where
// two independent messages are worth more than twice one -- see sha_hook.go.
//
...
func AstroBWTv3_pair(inputA, inputB []byte, scratchA, scratchB *ScratchData) (hashA, hashB [32]byte) {
	...
	lenB := astroStage(inputB, scratchB)

	// sa_bytes is only a view of sa on a little-endian machine, which is what
...
// falsify turns a panic into hashes that cannot meet any target.
//
// If something goes wrong due to RAM issues in a miner we should continue,
// avoiding crashes if possible; a random hash fails the check and the nonce is
// simply lost. Deferred directly, so its recover() is the one that catches a
// panic from the hash below it.
func falsify(out ...*[32]byte) {
	if r := recover(); r != nil {
...
// hashSuffixArray is the last step of a hash: SHA-256 over the suffix array as
// little-endian bytes.
func hashSuffixArray(scratch *ScratchData, data_len uint32, out *[32]byte) {
	if LittleEndian {
	...
	_ = scratch.hasher.Sum(out[:0])
}

// astroStage runs everything up to and including the suffix sort, and returns
// how many int32 of scratch.sa the final SHA-256 covers.
//
// Split out so that two nonces can reach the final SHA-256 together. It does not
// recover from panics itself; its callers do, which catches the same ones.
func astroStage(input []byte, scratch *ScratchData) uint32 {
	...
	// Suffix array of the accumulated state stream. This is ~93% of the hash
	// cost. text_32_fast is a redundant-work-free driver for the same SA-IS
	// algorithm and yields a byte-identical suffix array (see sais_fast.go).
	if CaptureStage1 != nil {
		CaptureStage1(scratch.data[:data_len])
	}

	// The hook first, when a miner installed one, and the built-in sort whenever
	// there is none or it declined. See sa_hook.go.
	if SuffixSort == nil || !SuffixSort(scratch.data[:data_len], scratch.sa[:data_len]) {
		lms, satmp := scratch.saisBuffers()
		text_32_fast(scratch.data[:data_len], scratch.sa[:data_len], lms, satmp)
	}
