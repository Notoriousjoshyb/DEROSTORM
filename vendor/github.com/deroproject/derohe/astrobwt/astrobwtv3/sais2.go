...
func induceSubL_32(text []int32, sa, freq, bucket []int32) {
	...
	// than cached in a register across iterations -- see induceSubL_8_32 in
	// sais.go for the measurement. Here the table is textMax entries rather
	// than 256, so it is tens of kilobytes and lives in L2, and the change is
	// worth less than at the byte level; it is still worth taking.
	{
		b := bucket[c1]
		bucket[c1] = b + 1
		sa[b] = int32(k)
