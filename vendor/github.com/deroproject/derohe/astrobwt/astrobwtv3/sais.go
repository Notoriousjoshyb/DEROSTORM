...
func induceSubL_8_32(text []byte, sa, freq, bucket []int32) {
	...
	// The bucket cursor is read and written straight through the table rather
	// than cached in a register across iterations. The stock code kept a copy
	// and only touched the table when the character changed, which trades an L1
	// access for a branch -- and the character here is RC4-derived, so that
	// branch mispredicts constantly. Measured +22% on one thread, +3% on
	// sixteen (SMT was already hiding the mispredicts).
	{
		b := bucket[c1]
		bucket[c1] = b + 1
		sa[b] = int32(k)
